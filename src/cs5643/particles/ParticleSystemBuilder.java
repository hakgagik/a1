...
public class ParticleSystemBuilder implements GLEventListener
{
    ...
    private static int N_STEPS_PER_FRAME = 2;
    ...
    public static final double DT = 0.016667;
        ...
        public void dispatchKey(KeyEvent e)
        {
            switch(e.getKeyCode()) {
                ...
                case KeyEvent.VK_I:
                    ...
                    break;
	            case KeyEvent.VK_N:
		            java.util.Random rf = new java.util.Random();
		            Point3d x0 = new Point3d(rf.nextFloat(), rf.nextFloat(), rf.nextFloat());
		            Particle lastCreatedParticle = PS.createParticle(x0);
		            break;
	            case KeyEvent.VK_M:
		            rf = new java.util.Random();
		            for (int i = 0; i < 1000; i ++) {
			            x0 = new Point3d(rf.nextFloat(), rf.nextFloat(), rf.nextFloat());
			            lastCreatedParticle = PS.createParticle(x0);
		            }
		            break;
                case KeyEvent.VK_L:
                    loadFrameFromFile();
                ...
                case KeyEvent.VK_UP:
	                ...
	                vec3.sub(eyePos, targetPos);
	                double r = vec3.length();
	                double phi = Math.acos(vec3.y / r);
	                double theta = Math.atan2(vec3.x, vec3.z);
	                phi -= Constants.DPHI;
	                if (phi > Math.PI+0.001) phi = Math.PI+0.001;
	                eyePos.x = r * Math.sin(phi) * Math.sin(theta) + targetPos.x;
	                eyePos.y = r * Math.cos(phi)                   + targetPos.x;
	                eyePos.z = r * Math.sin(phi) * Math.cos(theta) + targetPos.z;
                    break;
                case KeyEvent.VK_DOWN:
	                ...
	                theta = Math.atan2(vec3.x, vec3.z);
	                phi += Constants.DPHI;
                ...
                case KeyEvent.VK_COMMA:
                    ...
                    try {
                        ...
                        for(int i=0; i<numParticles; i++) {
                            ...
                            if (y > 0.5) PS.createParticle(new Point3d(x, y, z));
                        }
                        s.close();
